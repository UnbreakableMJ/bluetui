...
impl App {
    ...
    pub fn render_controllers(
        &mut self,
        controller_block: Rect,
        selected_controller_index: usize,
        frame: &mut Frame,
    ) {
    ...
    fn render_new_devices(
        &mut self,
        new_devices_block: Rect,
        selected_controller_index: usize,
        frame: &mut Frame,
    ) {
        let selected_controller = &self.controllers[selected_controller_index];
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        ...
        if let Some(selected_controller_index) = self.controller_state.selected() {
            ...
            self.render_paired_devices(paired_devices_block, selected_controller_index, frame);

            if render_new_devices {
                self.render_new_devices(new_devices_block, selected_controller_index, frame);
            }

            self.render_controllers(controller_block, selected_controller_index, frame);
